...
namespace Back_end.Services
{
    public class ESIService
    {
        ...
        public async Task<List<StructureItem>> GetStructureIdList()
        {
            ...
            // ------ POS bases ------
            //var starbaseIndexResponse = await _client.Corporation.Starbases();
            //starbaseList.AddRange(starbaseIndexResponse.Data);

            //for (var i = 2; i <= starbaseIndexResponse.Pages; i++)
            //{
            //    var itemPage = await _client.Corporation.Starbases(i);
            //    starbaseList.AddRange(itemPage.Data);
            //}

            //starbaseList.ForEach(async starbase =>
            //{
            //    var starbaseResponse = await _client.Corporation.Starbase(starbase.StarbaseId, starbase.SystemId);
            //    var starbaseItem = starbaseResponse.Data;

            //    //CONTINUE

            //    var fuelBlockLevel = 0;
            //    foreach(var fuelType in starbaseItem.Fuels)
            //    {
